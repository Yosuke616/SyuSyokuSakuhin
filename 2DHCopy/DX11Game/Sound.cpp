...
IXAudio2SubmixVoice*	CSound::m_pSubmixBGM = nullptr;
IXAudio2SubmixVoice*	CSound::m_pSubmixSE	 = nullptr;

XAUDIO2_SEND_DESCRIPTOR BGM_Send;
XAUDIO2_VOICE_SENDS		BGM_SendList;

XAUDIO2_SEND_DESCRIPTOR SE_Send;
XAUDIO2_VOICE_SENDS		SE_SendList;
...
void CSound::Init(void)
{
	...
	if (!m_pXAudio2) {
...
#endif
			hr = m_pXAudio2->CreateMasteringVoice(&m_pMasteringVoice);	//	マスターボリューム
			if (FAILED(hr)) {
		...
		//	BGM
		m_pXAudio2->CreateSubmixVoice(&m_pSubmixBGM, 1, 44100, 0, 0, 0, 0);	//	BGMボリューム
		BGM_Send = { 0, m_pSubmixBGM };
		BGM_SendList = { 1, &BGM_Send };

		//	SE
		m_pXAudio2->CreateSubmixVoice(&m_pSubmixSE, 1, 44100, 0, 0, 0, 0);	//	SEボリューム
		SE_Send = { 0, m_pSubmixSE };
		SE_SendList = { 1, &SE_Send };
	}
	if (m_pXAudio2) {
...
void CSound::Fin(void)
{
	...
	SAFE_DESTROYVOICE(m_pSubmixBGM);
	SAFE_DESTROYVOICE(m_pSubmixSE);
	SAFE_DESTROYVOICE(m_pMasteringVoice);
	SAFE_RELEASE(m_pXAudio2);
...
// SE再生中？
bool CSound::IsPlaying(eSE se)
{
	if (!m_pSe || se < 0 || se >= MAX_BGM) return false;
	return m_pSe[se].IsPlaying();
}
...
HRESULT CSoundStream::Open(IXAudio2* pXAudio2, LPCWSTR pszPath)
{
	...
	hr = pXAudio2->CreateSourceVoice(&m_pSourceVoice, &wfx, XAUDIO2_VOICE_NOPITCH, 1.0f, nullptr, &BGM_SendList);
	return hr;
}
...
HRESULT CSoundEffect::Open(IXAudio2* pXAudio2, LPCWSTR pszPath)
{
	...
	for (int i = 0; i < MAX_DUP; ++i) {
		hr = pXAudio2->CreateSourceVoice(&m_pSourceVoice[i], &wfx, XAUDIO2_VOICE_NOPITCH, 1.0f, nullptr, &SE_SendList);
		if (FAILED(hr)) {
			return hr;
		}
	}
...
// BGMボリューム設定
void CSound::SetVolume_BGM(float fVol)
{
	m_pSubmixBGM->SetVolume(fVol);
}

// SEボリューム設定
void CSound::SetVolume_SE(float fVol)
{
	m_pSubmixSE->SetVolume(fVol);
}

// BGMボリュームの値
float CSound::GetVolume_BGM()
{
	float fVol = 0.0f;
	if (m_pSubmixBGM) {
		m_pSubmixBGM->GetVolume(&fVol);
	}
	return fVol;
}

// SEボリュームの値
float CSound::GetVolume_SE()
{
	float fVol = 0.0f;
	if (m_pSubmixSE) {
		m_pSubmixSE->GetVolume(&fVol);
	}
	return fVol;
}