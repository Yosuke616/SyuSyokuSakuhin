...
void DrawMesh(ID3D11DeviceContext* pDeviceContext, MESH* pMesh, int nTranslucntType)
{
	...
	SHADER_GLOBAL cb;
	XMMATRIX mtxWorld = XMLoadFloat4x4(&pMesh->mtxWorld);	//	メッシュのワールドマトリックスを格納
	CCamera* pCamera = CCamera::Get();						//	カメラの取得

	//	ワールド×ビュー×射影行列(転置行列)
	cb.mWVP = XMMatrixTranspose(
		mtxWorld
	...
	//	メッシュのテクスチャマトリックスをセット
	cb.mTex = XMMatrixTranspose(
		XMLoadFloat4x4(&pMesh->mtxTexture));
